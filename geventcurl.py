...
for key, value in pycurl.__dict__.items():
...
class GeventCurl(object):
    ...
    def __init__(self, loop=None):
        ...
        try:
            ...
            # python).
            logging.warning(
                "socket_action method missing from pycurl; "
                "falling back to socket_all. Upgrading "
                "libcurl and pycurl will improve performance"
            )
            self._socket_action = lambda fd, action: self._obj.socket_all()
    ...
    def _set_socket(self, event, fd, multi, data):
        ...
        if event == pycurl.POLL_REMOVE or not event:
            ...
            if watcher is None:
                ...
                watcher.start(self._handle_events, watcher.events, fd)
...
class Curl(object):
    ...
    def perform(self):
        assert (
            getattr(self._obj, "waiter", None) is None
